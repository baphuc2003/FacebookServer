...
import { IPostType, IPostAudience, IMediaType } from '~/models/database/post'
...
export const createPostValidator = checkSchema(
  {
    ...
    content: {
      custom: {
        options: (value: string, { req }) => {
          ...
          //if type is Repost
          if (type.toLowerCase() === 'repost' && value !== '') {
            throw new ErrorWithStatus({
              status: 400,
    ...
    hashtags: {
      custom: {
        options: (value: string[], { req }) => {
          const type = req.body.type as string
          console.log('check 133 ', !(value === null))
          if (type.toLowerCase() === 'repost' && !(value === null)) {
            throw new ErrorWithStatus({
              status: 400,
              message: 'Hashtags must be a null'
            })
          }
      ...
      custom: {
        options: async (value: string[], { req }) => {
          ...
          if (type.toLowerCase() === 'repost' && !(value == null)) {
            throw new ErrorWithStatus({
              status: 400,
              message: 'Mentions must be a null'
            })
          }
